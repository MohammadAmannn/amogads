# AmogaDS — Central Design System (`@amoga/ui`)

Welcome to the **Amoga Design System (AmogaDS)** repository. This repository serves two unified purposes:
1. **Source of Truth for `@amoga/ui`**: A centralized, versioned design system package containing design tokens, atomic UI primitives, reusable business patterns, and page templates.
...
## 📦 Consuming `@amoga/ui` in Next.js Applications

### 1. Configure Package Registry
Add `@amoga` scope to your project's `.npmrc`:
```ini
@amoga:registry=https://npm.pkg.github.com
//npm.pkg.github.com/:_authToken=${GITHUB_TOKEN}
```

### 2. Install Package
```bash
npm install @amoga/ui
```
...
```css
@import "tailwindcss";
@import "@amoga/ui/theme.css";
```

### 4. Consume Components in Code
```tsx
import { 
  Button, 
  PageHeader, 
...
|---|---|---|
| **Tokens** | `@amoga/ui/tokens`<br/>`@amoga/ui/theme.css` | `SEMANTIC_TOKENS`, `CHART_TOKENS`, `SIDEBAR_TOKENS`, CSS custom properties |
| **Primitives** | `@amoga/ui` | `Button`, `Input`, `Card`, `Table`, `Tabs`, `Dialog`, `Sheet`, `Drawer`, `Select`, `Checkbox`, `RadioGroup`, `Slider`, `Switch`, `Badge`, `Avatar`, `Breadcrumb`, `DropdownMenu`, `Popover`, `Tooltip`, `Command`, `ContextMenu`, `Pagination`, `Progress`, `ScrollArea`, `Skeleton`, `Sonner`, `Toaster`, `Spinner`, `Textarea`, `Toggle`, `Kbd`, `Label`, `Empty`, `Field`, `Form`, `InputGroup`, `InputOTP` |
| **Business Components** | `@amoga/ui` | `PageHeader`, `DataTable`, `StatusBadge`, `FilterBar`, `FormSection`, `MetricCard`, `ConfirmDialog`, `PasswordInput`, `SignOutDialog`, `ThemeSwitch`, `ThemeSelector`, `SelectDropdown`, `LongText`, `Search`, `Stats01`, `DatePicker`, `QrCodeDisplay` |
| **Page Templates** | `@amoga/ui` | `ListTemplate`, `DetailTemplate`, `FormTemplate`, `WizardTemplate`, `DashboardTemplate`, `WorkspaceTemplate` |
...
### 3. Build Distribution Package (`@amoga/ui`)
```bash
...
5. Publishes `@amoga/ui` to GitHub Packages registry.
...
AmogaDS provides a fully controlled, automated cross-repository dependency upgrade pipeline to keep all consuming applications up to date with `@amoga/ui` releases without risk of accidental code overwrites or regressions.

```
@amoga/ui Release Published (e.g. v1.4.0)
  ↓
...
```text
===============================================================
  🌟 AmogaDS (@amoga/ui) Central Consumer Tracker
...
To register a new Next.js application for automated `@amoga/ui` upgrades, add an entry to [`consumers-registry.json`](consumers-registry.json):
...
npm install @amoga/ui@<PREVIOUS_VERSION>

# 2. Re-build and verify
npm run build

# 3. Commit and push
git commit -am "fix: rollback @amoga/ui to v<PREVIOUS_VERSION>"
