...
1. **Use Semantic Tokens**: Always use semantic Tailwind color utility classes (e.g. `bg-background`, `text-foreground`, `border-border`, `bg-card`, `text-muted-foreground`, `bg-primary`) rather than hardcoded hex or RGB colors.
2. **Reuse Existing Components**: Always import core UI primitives from `@/design-system` (or `@/components/ui/...`). Do not create duplicate Button, Input, or Card components.
